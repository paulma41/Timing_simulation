...
ActionTyped = Tuple[int, Event, Event]  # (type_idx, (e_val, t_e), (r_val, t_r))
Action = Tuple[Event, Event]  # ((e_val, t_e), (r_val, t_r))
...
def _build_forced_actions(
    ...
    *,
    ...
    dt_rew: float = 20.0,
) -> Tuple[List[Tuple[float, float]], List[ActionTyped], List[Action], List[Event], List[Event]]:
    """
    Build deterministic actions from a forced sequence of action types.
    ...
    return action_types, A_typed, A, Eff_, Rew_


def main() -> None:
    # Forced action types: [1,1,1,2,2,2,3,3,3]
    type_seq = [1, 1, 1, 2, 2, 2, 3, 3, 3]
    action_types, A_typed, A, Eff_, Rew_ = _build_forced_actions(type_seq)

    models = []
    for kernel in ["event_weighted", "action_avg"]:
        for update in ["continuous", "event", "action"]:
            # Old/legacy implementation from functions/h_actions.py (includes analytic jacobian).
    ...
    ax.set_title("h(t) for 6 legacy models (functions/h_actions.py) on forced action sequence [1,1,1,2,2,2,3,3,3]")
    ax.set_xlabel("t")
    ax.set_ylabel("h(t)")
